...
import { mdiDotsVertical, mdiFileImageOutline } from "@mdi/js";
...
export const chatAvatarModalButton = new Button({
...
export const chatMenuModals = function (currentChat: chatsTypes.TChat) {
  return [
    new Modal({
      ...
      ref: "chatMenuModal",
      body: new List({
        items: [chatAvatarModalButton].map((entry) => new ListItem({ body: entry })),
        className: "list-menu",
      }),
    }),
    new Modal({
